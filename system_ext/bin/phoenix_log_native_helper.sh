...
function phx_native_generate_keyfinfo()
{
    ...
    echo "phoenix timestamp:" >> ${key_info_file}
    echo T > /proc/phoenix
    cat /proc/phoenix >> ${key_info_file}
    echo T > /proc/phoenix
    echo "${key_info_file}" >> ${key_info_file}
}
...
function is_euex_std_product() {
  #region="`getprop ro.oplus.image.my_heytap.type`"
  region="`getprop ro.oplus.pipeline.region`"
  if [ "${region}" == "EUEX" ] || [ "${region}" == "ROW" ] || [ "${region}" == "US" ] || \
  [ "${region}" == "TMO" ] || [ "${region}" == "JP" ]; then
    echo "BE CAREFUL ... EUEX NA or JP Enter !"
    return 1
  else
    return 0
  fi
...
function phx_log_native_helper_main()
{
    ...
    do
        ...
        then
            ...
            # olc copy phoenix log start
            if [ "${phx_error}" == "ERROR_REBOOT_FROM_KE_SUCCESS" -a "x${HARDWARE}" != "xqcom" ]; then
                olc_phoenix_size=$(du -sk ${TMP_DIR_FOR_OLC} | cut -f1)
                if [ -e ${TMP_DIR_FOR_OLC}/ERROR_KERNEL_EXCEPTION -a ${olc_phoenix_size} -gt 10240 ]; then
                    rm -rf ${TMP_DIR_FOR_OLC}
                else
                    echo "compatible for mtk platform ERROR_REBOOT_FROM_KE_SUCCESS"
                fi
            else
                rm -rf ${TMP_DIR_FOR_OLC}
            fi
