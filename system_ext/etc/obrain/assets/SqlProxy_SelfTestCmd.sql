...
AGG_CPU_DAILY_CHECK_TAB as (
	select 'agg_cpu_app_daily_data_check' as item
    	, case
    		when count(*) > 0 then 'fail'
    		else 'pass'
    	end as result, ifnull(fail_cnt, 0) as fail_cnt, detail
    from (
    	select fail_cnt, json_group_array('agg_cpu_app_daily :' || detail) as detail
    	from (
    		select count(*) as fail_cnt
...
AGG_GPU_DAILY_TAB as (
	...
	from (
		select case
			...
			end as duration_check, usage
			, case
				when usage >= 0 then 'pass'
				else 'fail'
			end as usage_check, energy
			, case
...
COMP_GPU_AGENT_RESULT_TAB as (
	...
	from (
		select a.fail_cnt + c.fail_cnt + b.fail_cnt as fail_cnt
			, json_group_array('comp_gpuPower_gpuAgent_intv :' || c.detail || a.detail || b.detail) as detail
		from (
			select count(*) as fail_cnt
...
AGG_GPU_APP_DAILY_RESULT_TAB as (
	...
	from (
		...
		from (
			select count(*) as fail_cnt
				, json_group_array('empty_check :' || empty_check || ', timestamp :' || timestamp || ':' || timestamp_check || ', start_ts :' || start_ts || ':' || start_ts_check ||
				', end_ts :' || end_ts || ':' || end_ts_check || ', name :' || name_check || ', duration :' || duration_check ||
				', usage :' || usage_check || ', energy :' || energy || ':' || energy_check) as detail
...
union
select * from AGG_GPU_APP_DAILY_RESULT_TAB
union
...
union
select * from AGG_CPU_DAILY_CHECK_TAB
union
