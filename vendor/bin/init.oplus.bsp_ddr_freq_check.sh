...
do
    if [ "$element" = "QCOM" ]; then
        ...
        if  [ $PLAT == "QCOM" ]; then
            ...
            line=$opp$p
            echo "line is $line"
            if [ "$platform" = "mt6991" ] || [ "$platform" = "mt6899" ]; then
                DVFSRC_OPP_TABLE_LINE=$(cat ${DVFSRC_PATH}dvfsrc_opp_table | sed -n $line | cut -c15-24)
                DDR_FREQ=$(cat ${DVFSRC_PATH}dvfsrc_opp_table | sed -n $line | cut -c15-24 | sed 's/[^0-9]//g')
            else
