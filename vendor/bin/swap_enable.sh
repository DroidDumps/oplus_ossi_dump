...
function configure_para_aladdin()
{
	...
	case "$product_id" in
		...
		"2471"[3-5]|"24728")
			...
			if [ $MemTotal -le 6291456 ]; then
				...
				echo 39300 > /sys/module/oplus_bsp_zram_opt/parameters/oplus_extra_free_kbytes
			else
				echo 25 > /proc/sys/vm/watermark_scale_factor
				echo 63300 > /sys/module/oplus_bsp_zram_opt/parameters/oplus_extra_free_kbytes
			fi
			echo 24748 > /proc/sys/vm/min_free_kbytes
			;;
		#for Oris-C
		"2470"[0-2]|"24709")
			echo 160 > /proc/sys/vm/swappiness
			...
			if [ $MemTotal -le 6291456 ]; then
				echo 10 > /proc/sys/vm/watermark_scale_factor
				echo 63787 > $para_path/oplus_extra_free_kbytes
				echo 18944 > /proc/sys/vm/min_free_kbytes
			else
				echo 23 > /proc/sys/vm/watermark_scale_factor
				echo 66304 > $para_path/oplus_extra_free_kbytes
				echo 22154 > /proc/sys/vm/min_free_kbytes
		...
		"226AD"|"226AE"|"226AF")
			if [ $MemTotal -le 4194304 ]; then
				echo 10 > /proc/sys/vm/watermark_scale_factor
				echo 35000 > $para_path/oplus_extra_free_kbytes
				echo 15484 > /proc/sys/vm/min_free_kbyte
			elif [ $MemTotal -le 6291456 ]; then
				echo 10 > /proc/sys/vm/watermark_scale_factor
				echo 61150 > $para_path/oplus_extra_free_kbytes
				echo 19210 > /proc/sys/vm/min_free_kbytes
			else
				echo 25 > /proc/sys/vm/watermark_scale_factor
				echo 63300 > $para_path/oplus_extra_free_kbytes
				echo 22270 > /proc/sys/vm/min_free_kbytes
			fi
			;;
		#for Ark-A
		"23703"|"23704")
			if [ $MemTotal -le 6291456 ]; then
				echo 10 > /proc/sys/vm/watermark_scale_factor
				echo 39200 > $para_path/oplus_extra_free_kbytes
				echo 24748 > /proc/sys/vm/min_free_kbytes
			else
				echo 25 > /proc/sys/vm/watermark_scale_factor
				echo 63300 > $para_path/oplus_extra_free_kbytes
...
function oplus_configure_hybridswap() {
	...
	if [[ "$kernel_version" == "5.1"* || "$kernel_version" == "6.1"* || "$kernel_version" == "6.6"* ]]; then
...
function oplus_configure_tuning_swappiness() {
	...
	if [ $MemTotal -le 6291456 ]; then
		...
		echo 1500 > $para_path/swappiness_threshold2_size
		echo 25 > $para_path/watermark_scale_factor
	elif [ $MemTotal -le 12582912 ]; then
		echo 100 > $para_path/vm_swappiness_threshold1
		echo 3600 > $para_path/swappiness_threshold1_size
		echo 120 > $para_path/vm_swappiness_threshold2
		echo 1500 > $para_path/swappiness_threshold2_size
		echo 25 > $para_path/watermark_scale_factor
	fi
